...
public class JwtUtils {
    ...
    public String getUsernameFromJWT(String jws) {
        return Jwts.parserBuilder().setSigningKey(jwtSecret).build().parseClaimsJws(jws).getBody().getSubject();
    }

    public boolean validateToken(String jws) {
        try {
            Jwts.parserBuilder().setSigningKey(jwtSecret).build().parseClaimsJws(jws);
            return true;
